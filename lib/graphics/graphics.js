...
import { light_point, resetLightIndexes, setNrLights } from "./light.js";
...
function draw(cT) {
    ...
    lights[0].position = vec3.fromValues(Math.sin(cT / 1000) * 10, 0, 0);
    resetLightIndexes();
...
function main() {
    ...
    lights.push(new light_point(vec4.fromValues(0.1, 0.1, 0.1, 1.0), vec4.fromValues(0.2, 0.2, 0.8, 1.0), vec4.fromValues(0.5, 0.5, 0.5, 1.0), vec3.fromValues(0, 0, 0)));
    lights.push(new light_point(vec4.fromValues(0.1, 0.1, 0.1, 1.0), vec4.fromValues(0.8, 0.2, 0.2, 1.0), vec4.fromValues(0.5, 0.5, 0.5, 1.0), vec3.fromValues(0, 1, 10)));
    addModel(modelName);
    // for(let i = 0; i < 2; i++)
    // {
