...
vec4 calcDirectionalLight(light_directional light, vec3 normal, vec3 viewDir)
{
    ...
    vec4 specularResult =  max(vec4(0.0), vec4(specular, 1.0f) * texture(mat_specular, frag_uv));

    // if(specularResult.x < 0.0)
    // {
...
function draw(cT) {
    ...
    //(lights[0] as light_point).position = vec3.fromValues(Math.sin(cT/1000)*75, 20, Math.cos(cT/2000)*75);
    ...
    for (let i = 0; i < lights.length; i++) {
        if (lights[i] instanceof light_point) {
            ...
            let x = Math.sin((((i * lights.length) / circ) + (cT / 2000))) * 25;
            let z = Math.cos((((i * lights.length) / circ) + (cT / 2000))) * 25;
            //console.log(x + "," + z);
            lights[i].position = vec3.fromValues(x, 20, z);
...
function main() {
    ...
    for (let i = 0; i < 12; i++) {
        lights.push(new light_point(gl, vec4.fromValues(0.1, 0.1, 0.1, 1.0), vec4.fromValues(i % 3 == 0 ? 0.8 : 0.2, i % 3 == 1 ? 0.8 : 0.2, i % 3 == 2 ? 0.8 : 0.2, 1.0), vec4.fromValues(0.5, 0.5, 0.5, 1.0), vec3.fromValues(0, 20, 0)));
    }
    lights.push(new light_directional(gl, vec4.fromValues(0.03, 0.03, 0.03, 1.0), vec4.fromValues(0.0, 0.0, 0.0, 1.0), vec4.fromValues(0.0, 0.0, 0.0, 1.0), vec3.normalize(vec3.fromValues(0, 0, 0), vec3.fromValues(-1.0, -1.0, -1.0))));
    ...
    models.push(new model("de_dust2", gl, program, vec3.fromValues(25, 0, 25), vec3.fromValues(270, 0, 0), vec3.fromValues(0.05, 0.05, 0.05)));
