...
function draw(cT:number)
{
    ...
    for(let i = 0; i < lights.length; i++)
    {
        if(lights[i] instanceof light_point)
        {
            ...
            //console.log(x + "," + z);
            (lights[i] as light_point).position = vec3.fromValues(x, 20, z)
...
function addModel(name:string, position?:vec3, rotation?:vec3, scale?:vec3)
{
}
...
function main()
{
    ...
    lights.push(new light_directional(
        gl,
        vec4.fromValues(0.03, 0.03, 0.03, 1.0),
        vec4.fromValues(0.0, 0.0, 0.0, 1.0),
        vec4.fromValues(0.0, 0.0, 0.0, 1.0),
        vec3.normalize(vec3.fromValues(0,0,0), vec3.fromValues(-1.0, -1.0, -1.0))
    ))
